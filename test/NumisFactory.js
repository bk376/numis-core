...
// Start test block
describe("NumisFactory", async () => {
  const [deployer] = await ethers.getSigners();
  console.log("Deployer is mr. :", deployer.address);
  before(async function () {
    this.Factory = await ethers.getContractFactory("NumisFactory");
    this.MockContract = await ethers.getContractFactory(
      "contracts/MockContract.sol:MockContract"
    );
  });
